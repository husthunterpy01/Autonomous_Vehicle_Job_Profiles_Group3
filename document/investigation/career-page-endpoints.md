...
| 10 | Bosch | [Bosch jobs](https://jobs.smartrecruiters.com/BoschGroup) | `SR(BoschGroup) - https://api.smartrecruiters.com/v1/companies/BoschGroup/postings?limit=100&offset=0&utm_source=chatgpt.com` | SR detail with company ID `BoschGroup` and `{posting_id}`. | Preferred: SmartRecruiters JSON. This matches the existing Bosch scraper. |
