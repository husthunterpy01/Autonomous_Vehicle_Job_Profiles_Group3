...
def _salary_fields(row: dict, company_salary_cache: dict[str, dict]) -> dict:
    ...
    Phase 1/2a produce a real disclosed min-max range for *this* posting, so
    they fill salary_min/salary_max. Phase 2b is a single company-wide
    median across all roles and levels (see LevelsFyiAverage) with no real
    range at all - it fills the separate salary_average column instead of
    duplicating that one number into both salary_min and salary_max, which
    previously made an estimate look like a precise (and suspiciously exact,
    min == max) disclosed range.

    Phase 1 also requires salary_currency and salary_period, not just the
    two numbers - salary_sync.sync_salary rejects a row missing either (the
    Greenhouse bronze model can produce a real salary_min/salary_max with a
    null salary_currency when the source posting's pay_input_ranges entry
    has no currency_type), and import_salary rolls back its *entire* batch
    on one row's ValueError. Falling through to regex/levels.fyi here keeps
    an incomplete Phase 1 row from taking down every other job's salary
    import in the same run.
    """
    if (
        row.get("salary_min") is not None
        and row.get("salary_max") is not None
        and row.get("salary_currency")
        and row.get("salary_period")
    ):
        return {
