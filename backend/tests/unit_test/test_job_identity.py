...
from app.validate_handoff import validate_records
...
def test_source_id_scoped_by_ats(db_session):
    ...
    second = seed(db_session, "two", ats_name="lever")
...
def test_same_ats_collisions_require_silver_key(db_session):
    seed(db_session)
    second = seed(db_session, "two", company_name="Other AV")
    with pytest.raises(ValueError, match="Ambiguous"):
        resolve_job(db_session, {"source_job_id": "42", "ats_name": "greenhouse"})
    assert resolve_job(db_session, {"deduplication_key": "two"}) == second


@pytest.mark.parametrize("identity", [
    {"job_id": "1"}, {"bronze_id": 1}, {"source_job_id": "42"},
    {"deduplication_key": ""}, {"deduplication_key": "missing"},
    {"deduplication_key": "one", "source_job_id": "wrong"},
    {"deduplication_key": "one", "ats_name": "lever"},
    {"deduplication_key": "missing", "source_job_id": "42", "ats_name": "greenhouse"},
])
...
def test_preflight_reports_pending_category_and_makes_no_changes(db_session):
    job = seed(db_session)
    db_session.commit()
    report = validate_records(db_session, [{"deduplication_key": "one", "functional_area": "Perception"}])
    ...
    with pytest.raises(ValueError, match="Row 2.*same backend job"):
        validate_records(db_session, [{"deduplication_key": "one"}] * 2)
